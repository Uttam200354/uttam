    ...
    assetsData.forEach(asset => {
        ...
        row.innerHTML = `
            ...
            <td>
                ...
                <button class="btn btn-danger" onclick="deleteAsset(this)">Delete</button>
            </td>
        `;
...
function deleteAsset(button) {
    deleteRow(button, 'assets', loadAssetsTable);
}
    ...
    plantData.forEach(plant => {
        ...
        row.innerHTML = `
            ...
            <td>
                ...
                <button class="btn btn-danger" onclick="deletePlantData(this, '${plantType}')">Delete</button>
            </td>
        `;
        tableBody.appendChild(row);
    });
...
// Delete plant data
function deletePlantData(button, plantType) {
    if (confirm('Are you sure you want to delete this record?')) {
        const row = button.closest('tr');
        const srNo = row.querySelector('td').textContent.trim();
        
        // Get data from storage
        const data = getStorageData(`plant_${plantType}`);
        const updatedData = data.filter(item => item.srNo !== srNo);
        
        // Save updated data
        setStorageData(`plant_${plantType}`, updatedData);
        
        // Reload table
        loadPlantTable(plantType);
        
        showSuccessMessage('Plant record deleted successfully!');
    }
}
